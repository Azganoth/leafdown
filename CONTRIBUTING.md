...
Targets are `block-selection`, `diagnostics`, `document-lifecycle`, `folder-watcher`, `folder-actions`, `rendered-images`, `remote-images`, `rendered-html`, `separator-presentation`, `missing-document-error`, `persistence`, and `window-lifecycle`. Focused runs use the same isolation, evidence capture, and cleanup as the full suite. The `:run` command does not check binary freshness; run `pnpm build:e2e:desktop` when its inputs change.
...
Verify changes locally before merging. Use `pnpm check:frontend` for frontend-only work, `pnpm check:backend` for Rust/Tauri-only work, and `pnpm check` for cross-cutting updates. For manual testing of Markdown and the article navigator, open the committed `corpus/` directory or one of its focused scenario directories in the app. Markdown round trips over `corpus/commonmark/`, `corpus/gfm/`, and `corpus/isolated/end-of-file/` are asserted automatically by `src/features/editor/tests/corpusRoundTrip.test.ts`: a file whose every form the editor can carry is asserted to write back byte for byte, a file a normalized form holds short of that is asserted to converge instead, and both are asserted to reopen as the document that wrote them. The manual pass over those directories therefore covers rendering, interaction, and navigator behavior; the remaining subtrees are verified only by hand.
