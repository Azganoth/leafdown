...
The shared projection engine owns the active session, projected range, projection-local history, dirty-state integration, and finalization. A session covers one object, or the pair of adjacent objects a caret sits between, which it holds as one range with one adapter and one history. A clean session restores its original content; an edited session rehydrates valid source or commits literal text so projected characters are not discarded. A projected range holds flat text; a session whose range stops holding it ends without committing, leaving what landed there in the document.

A construct an input rule has just built does not project, because the caret the rule leaves against it is indistinguishable by position from a caret moved there, and projecting it would take the author's next character into the construct they just closed. It projects like any other object once the caret returns to it.

Document text that already spells a supported object, rather than having been projected from one, commits through the same adapter validation when the caret leaves it. The engine commits only inside the ranges the session has written, and never inside a run of source the file escaped, which it recognizes from the state a write lands in before that write changes it. A change that only relocates content the document already held, such as a table row move, declares itself and records nothing as written, because its steps re-insert what they took. An escape projection records nothing as written for the same reason: spelling out an escape the file already holds is not the session authoring the run. The same characters reach the document either way, because the escape does not survive parsing. History clears both records, so an undone commit stays undone.

A restructure reads canonical content, so it finalizes an active projection before it rebuilds what it carries. Projected source left standing is rebuilt as the text it spells, which every object adapter survives because its source parses back to what it replaced.
...
Marker presentation remains separate from projection lifecycle. Decorations style active source, while projected Markdown remains document text rather than widget or NodeView input state. A standalone image, or a logical link whose complete label is one image, inserts its source immediately before the original image node rather than replacing it for the duration of the session. The projected text remains the session's editable range, while the retained image stays outside that range and keeps its NodeView and decoded media. Pointer entry is carried through any restore and commit transactions separately from directional keyboard entry, so clicking can target the alt text without turning keyboard traversal over the retained node into another entry. Finalization removes the temporary source and, for an edit, replaces the retained original with the parsed result through the same two-phase history path used by other projections. Links with mixed or multiple label nodes keep the ordinary source-only link projection because their semantic target is the complete label rather than one retained image.
