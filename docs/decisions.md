...
- An indented code block's indentation is written as four spaces on every non-blank line, whatever the file spelled it with. The preset writes the block through a map that never sees the file, so nothing on the node names the spelling and it survives only in the slice the block was built from, which is where [issue #401](https://github.com/Azganoth/leafdown/issues/401) reads it. That record reaches the run standing between the prefix the containers spell and the first character of the block's value, and stops where the run overshoots the block's content column: a quote written `>` and a bullet written `-` each put their content at column 2 and the code's at column 6, which is no tab stop, so a tab spelling that indentation runs past it and the surplus columns arrive as the leading spaces of the block's value. Writing such a run back moves the content column unless the record also drops those characters, which makes a form record own content and fails the same test that keeps a fence inside a container from carrying indentation of its own. [Issue #399](https://github.com/Azganoth/leafdown/issues/399) measured both and settles the second as normalized, which is what holds the two tab lines in `corpus/commonmark/text-and-breaks.md` short of byte identity. The container is not what costs the first: a block at the document root, whose tab covers the four-column strip exactly, is rewritten the same way.
