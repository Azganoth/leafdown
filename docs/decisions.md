...
- What an editing command does to a form already recorded is not settled here. Two commands making the same level change to a heading disagree today, examined in [issue #359](https://github.com/Azganoth/leafdown/issues/359): the one that rebuilds the block from its level drops the form, and the one that carries the node's attributes through keeps it. The difference is which helper each command reached for rather than a decision about form, and it delivers the same unrequested rewrite through an editing gesture rather than through a save.
