...
- Whitespace at a line edge the parser trims is not written back. No node carries it, so no spelling of it reloads on its own, and the record that would supply it is the one the consequence below declines. Whitespace no parse trims is content and is written back as it was read. Whitespace a parse spends on a node rather than trimming is that node's form and is written back with it, which a run of two or more spaces closing a line is: it spells a hard break, and reloads as the break it spells. It is the one whitespace run at a line edge a node owns, and the ownership test reaches it however invisible it is on the line.
- A record carrying the whitespace a parse trims from a line edge is declined on payoff and on its fallback rather than on reach, which is what [issue #400](https://github.com/Azganoth/leafdown/issues/400) revisited once the benchmark was measured writing three such runs back unchanged. Reach is not the obstacle: the run stands inside the slice the block was built from, the same slice the continuation prefix is read from, so the per-line shape already established would carry it. What it buys is three lines of the forty-one Leafdown rewrites, all in `corpus/commonmark/text-and-breaks.md`, and no file reaches byte identity by writing them, because that file is held short of it whatever this class does by an escape, by the tabs the consequence above normalizes, and by the trailing tab a thematic break's own record trims. Against that, the class is one ordinary tooling removes independently, so writing the runs back moves a diff rather than removing one: this repository's own formatter deletes all three while keeping the no-break space beside them, which is the boundary Leafdown already draws, and `git diff --check` reports a restored run as an error. The fallback is the second cost and the deciding one. Every other preserved form withdraws on form, while each refusal this record needs answers content: a run of two or more spaces restored to a line another line follows spells a hard break the document never held, and a run of any length restored after a backslash break cancels it and leaves the backslash as text. Preserving would also cost the distinction [issue #341](https://github.com/Azganoth/leafdown/issues/341) exists to draw, which a reader observes today in the file itself, where a no-break space closing a line is written and the space beside it is not.
- A backslash holding a character literal is not a form the document owns, so it is normalized to the bare spelling wherever the construct it guards cannot form where it sits, which is the class [issue #251](https://github.com/Azganoth/leafdown/issues/251) retires one construct at a time. Nothing in the document model records it: `state.safe` emits it on the way out against the character alone, so a file gains backslashes its author never wrote, and the relaxation passes are what narrow that class back to the positions where a construct really opens. This is the one axis where Leafdown is deliberately less preserving than the benchmark. Measured against Typora 1.14.9 in September 2026 over thirty-six cases in three files, saved with no edit, after an edit that left a block's text unchanged, and after an edit that changed it: Typora carries each block's own source through all three and writes both spellings of one document back as it read them, so ``a \` b`` and ``a ` b``, and `\--` and `--`, each stay as authored, where Leafdown converges both on the bare one. It adds no backslash of its own in any of them, including to a paragraph it rewrote from its own model, so every bare spelling the relaxation produces is one the benchmark holds stable; what differs is the one-time normalization, which costs an authored backslash and no content, and leaves a file both editors then round-trip unchanged.
...
- A form decorating the boundary between two blocks is owned by the second of them, and is withdrawn against the document rather than against the file it came from. The blank line the serializer puts between two blocks is the separator every pair reads back as, so the form a file holds is the pair written without one, which [issue #324](https://github.com/Azganoth/leafdown/issues/324) preserves by recording it on the block that follows. It is the one preserved form whose fallback cannot be decided from the node carrying it: dropping a separator merges two blocks wherever the second no longer interrupts the first, and the block the pair is measured against is the innermost one before it rather than its sibling, because a blockquote and a list item both continue their last paragraph lazily. The separator is therefore settled in the assembled document, on the line the block was actually written with rather than on the construct it is: a setext heading opens on ordinary text, an item whose content begins below its marker leaves that marker alone on its line, and a run of hyphens underlines the paragraph above instead of breaking it. This is also the limit of the rule above about a construct an edit replaces — the form survives the edit because it decorates the pair rather than the construct, and the fallback is what answers for a pair that no longer reads back.
- A form belonging to a line inside a block rather than to the block itself is recorded per line on the block that holds those lines. The leading whitespace a continuation line was written with is the one such form, preserved in [issue #322](https://github.com/Azganoth/leafdown/issues/322) for a paragraph and in [issue #381](https://github.com/Azganoth/leafdown/issues/381) for a setext heading: a parse strips it and joins the lines into one text, so nothing on the node names it and it survives only in the slice of the file the block was built from. A setext heading's underline is one of the lines the record covers, because the run it spells belongs to the heading's own form while the whitespace before that run belongs to this one, and the two are independent axes on the same line. The canonical fallback is the prefix the containers spell, which is what a line the editor added is written under, and what a recorded prefix gives way to where it would open a quote the document no longer holds. Dropping a prefix costs nothing in the other direction, because every container that can hold a paragraph continues it lazily. This is the one preserved form keyed by where a line falls rather than by the construct it decorates, so an edit that adds or removes a line moves the later records onto other lines; that costs form rather than content, because each prefix is measured against the containers the document holds when it is written rather than against the ones the file had. The escape the serializer writes in place of the whitespace goes back with it: `state.safe` escapes a block marker wherever a line could open one, and a line standing four columns past its containers opens none. Such a marker keeps its escape where the indentation a run of nested items stacks cannot be told from the line's own, because the serializer writes that run as one width and the boundaries inside it are not recoverable from it.
- The indentation an item's own marker stands behind is a form of that item and is written back over the prefix the containers spell, which is the class [issue #390](https://github.com/Azganoth/leafdown/issues/390) preserves. It is the same axis as the consequence above and is keyed to the item rather than to the line, because the run cannot be split: a nested item stands past its container's content column, and a tab covering both the container's prefix and the item's own columns is one character the parse expands against the tab stops. The record therefore reaches every line the item writes, since moving the marker's column moves the column the item's content stands at with it, and a line inside the item that carries a record of its own is written from that one, which already holds the item's indentation. The canonical fallback is the prefix the containers spell, taken wherever the recorded columns are no longer the ones the document holds the item at: where a quote marker does not stand at the column the containers write theirs at, where the marker would land before the column they wrote or four columns past it, after an item in the same list carrying no record of its own, and on a list standing right after another in the same container. Each of those writes the marker inside another item, moves a quote out of the container holding it, or opens indented code, which costs content rather than form. The indentation a blockquote itself carries is therefore not preserved through this record even where the record holds it, because the run it reads is the whole line prefix and only the item's share of it is this record's to write; a quote written back at another column would also defeat the separator the block after it carries. An item written on the line its container's marker opens stands behind that marker rather than behind a prefix, and carries no indentation of its own.
- An indented code block's indentation is written back with the tab the file spelled it with, recorded on the block for each of the lines it holds, which is the class [issue #401](https://github.com/Azganoth/leafdown/issues/401) preserves. The preset writes the block through a map that never sees the file, so nothing on the node names the spelling and it survives only in the slice the block was built from. The record is marked in place and settled in the assembled document rather than carried to the handler, because a block writes its lines before its containers add the prefix the run has to be measured against. It carries a marker kind of its own rather than reusing the continuation record: that record's content is the serializer's to relax, and a backslash a line of code spells before a block marker is one the file wrote itself. The record covers the run standing between the prefix the containers spell and the first character of the block's value, and stops on two boundaries, each because a form record on this axis can reach content rather than only form. A run overshooting the block's content column is refused, because a quote written `>` and a bullet written `-` each put their content at column 2 and the code's at column 6, which is no tab stop: a tab spelling that indentation runs past it and the columns it overshoots arrive as the leading spaces of the block's value, so restoring the run would move the column that content stands at. [Issue #399](https://github.com/Azganoth/leafdown/issues/399) measured that boundary and settles it as normalized, which is what holds the two tab lines in `corpus/commonmark/text-and-breaks.md` short of byte identity. A run spelling anything but the quote markers its containers write is refused as well, because the record stands in place of the whole prefix and one reaching a marker another container owns writes that marker back over the one the document now holds; a two-digit ordered marker puts its content column on a tab stop, which is what makes such a run alignable and the refusal necessary. Neither sibling record can carry a marker, so that boundary is this record's alone, and the spelling of an item's own marker and padding stays the axis its record owns. A prefix spelled in spaces carries no record, because the canonical run reproduces it character for character.
- A construct the editor creates takes the form the document prevails in, and the defined default is what stands where the document names none, which is the class [issue #360](https://github.com/Azganoth/leafdown/issues/360) settles. A default fixed against the document made every insertion a small rewrite: a list added to a file whose lists are all `-` was written `*`, and a heading added to a file of setext headings `##`, so the mixture this decision exists to prevent arrived one construct at a time rather than all at once on first save. Prevailing is settled per axis of a form rather than per construct, because a heading holds three that move independently, and only the constructs an axis reaches answer for it: a setext heading spells neither of an ATX heading's runs, and an empty heading spells neither a separator nor an underline. One answer is agreement however few constructs give it, and two leave the default, because choosing between them would settle for the author a question the document does not. A run whose length answers to the construct holding it carries only its presence across, so a created heading is closed at the length of its own opening sequence and underlined at the width of its own content; a length carried from another heading would be a form no file wrote. The answer is settled onto the construct as it is created rather than at the save, which is what makes it that construct's own form: a document that later disagrees with itself does not move a construct already written in it. Settling it over the assembled document is also what reaches every command, input rule, and structural edit that can open one of these constructs, rather than each of them separately. A default is still what a fallback withdraws to, because the form it withdraws is one the document already holds rather than one being chosen for a construct that has none. The classes settled this way are a list's marker and a heading's three axes, which are the defaults the issue names; every other default is left fixed until a class is argued on its own terms, since inheriting a form is a judgment about that form rather than a rule a class joins by having a default.
...
- The classes still normalized once [issue #251](https://github.com/Azganoth/leafdown/issues/251) closed were measured against Typora 1.14.9 in September 2026 on the six corpus files they hold short of byte identity, rather than on a fixture assembled from them. Of the forty-one lines Leafdown rewrites, Typora rewrites one, the trailing tab closing a thematic break, which it trims exactly as Leafdown does, and it rewrites nothing Leafdown leaves alone. It keeps the other forty: the escapes on a destination's balanced parentheses and on every escapable punctuation character, the trailing spaces a parse trims from a paragraph, the tabs an indented code block inside a container was indented with, the single-tilde run, the escape a crossed nesting takes, and every ragged table row. The two nesting lines are not evidence either way, because Typora renders them with their outer markers literal and reads a document neither CommonMark nor Leafdown reads. The list therefore records what Leafdown's model cannot own rather than a bar the category accepts, and the thematic break is the one class where both editors rebuild the line from a node instead of carrying its source.
- Output-formatting customization remains deferred, though not because preservation answers it. The two are orthogonal and Typora ships both: preservation settles the form a file already holds, while a preference settles the form a construct is created with, which is the question [issue #360](https://github.com/Azganoth/leafdown/issues/360) answers from the document instead of from a setting. Inheritance was preferred to a preference panel because it needs no interface, answers every axis at once rather than one switch at a time, and follows the document the author is in rather than a global setting that may not match it. A document naming no form of its own, or disagreeing with itself about one, is still written in the serializer's own.
...
- The preset's single hard break spelling is overridden. Its `hardbreak` node records only whether the break is soft, so a break authored as a run of two or more spaces and one authored as a backslash parse into the same node and are both written `\`, rewriting every space-spelled break in a file on its first save. Leafdown records the run on the node, decided in [issue #388](https://github.com/Azganoth/leafdown/issues/388), read from the slice of the file the node was built from, which is the whole of a break because it holds no children, with the line ending that slice closes on trimmed off it. A break the editor creates carries `\`, which is also what a recorded run gives way to where a run of spaces would not be read back as a break. A run spells one only where the line it closes already holds a character of its own, so the fallback answers the two positions where it does not: a line opening on the run is blank and ends the block, and a run written against whitespace already there is trimmed into the break along with it, costing the text that wrote it. Neither position can be authored, because a file spelling one opens the break somewhere else or not at all, so both are reached only by an edit. That character is read off the tree rather than off the `before` the serializer hands the handler, which `mdast-util-to-markdown` leaves empty while it peeks the next node to escape the text ahead of it, and a peek disagreeing with the write would escape that text against a character the file never receives. A soft break is the same node with `isInline` set and stands for the line ending it was read from rather than for a break of any spelling, so it carries no run.
