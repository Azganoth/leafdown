...
- A form change is judged by everything it forces, not by the class it was filed under. A tilde code fence rewritten to backticks cannot carry a backtick in its info string, so it writes `&#x60;` and another tool reads an info string the author did not write; that is content, though it sits in a fence-form issue.
