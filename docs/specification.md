...
- Every eligible block has a local gutter aligned to its own logical start, including nested list items and blocks inside blockquotes. From the content outward it contains a block handle, a reserved insertion slot, and a passive source-marker slot. The document reserves the root gutter inside its own width, so controls appearing or disappearing move no text at any window width; nested gutters follow the indentation already carried by their blocks. A list marker or quote rail remains between the action gutter and the block's content.
