...
- Block handles carry accessible names but add no tab stop per block. A polite editor status announces the selected block type or block count while focus remains in the editor. Progressive keyboard entry, navigation, and exit use the same selection model and are specified separately.
