...
- A code span in a table cell is written with a backslash before each `|` its content holds, because a bare `|` closes the cell, and a span typed in a cell holds what the cell reads from its source, so typing `` `a \| b` `` there produces a span holding `a | b`. A cell reads a backslash back out of a code span only before a `|`, pairing backslashes from the left, so content holding an odd run of backslashes directly before a `|` has no spelling a cell reads back. Such content is written as it stands, which keeps the row and reads back without the last backslash of that run.
