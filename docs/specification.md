...
- A standalone image projects its complete Markdown source as editable document text. A caret entering from the left starts at the beginning of the source, a caret entering from the right starts at the end, and selecting the rendered image selects the complete source. A valid edit rehydrates one canonical image; an invalid or incomplete edit becomes the literal text its source spells, on the same escape rule as a link. The source preserves the description, reference form, destination form, and title marker the file holds. An image inside a link label remains part of the link's one semantic projection, while an image inside one exact, contiguous supported mark combination belongs to that marked fragment. The standalone projection includes `Choose image`, which changes its target through the same relative-path and outside-folder rules as an authored target without opening a second Markdown editor.
...
- Selecting a rendered or placeholder image exposes its complete Markdown source in place for editing the description, target, optional title, or reference form, and offers a file picker for changing its target.
