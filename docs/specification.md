...
- A projected character reference shows the character it names beside its source, in the marker colour the projection already uses for syntax, because the form and the character it produces are read together and a reference naming whitespace cannot otherwise be confirmed from the source. The character is presentation only: it is not document text, takes no caret, is never selected, copied, or saved, and moves no offset between the document and the file. It is shown wherever a reference is projected, so the reference's own projection, a marked fragment holding one, and a link label holding one all present it the same way. Source that spells no reference shows none, which covers a form an edit has broken and text a backslash or a code span keeps literal.
